...
The `home-routes` handler in the `josh-web.routes.home` namespace
...
The `home-page` function will in turn call the `josh-web.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `josh-web.handler` namespace:
...
Request middleware functions are located under the `josh-web.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `josh-web.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `josh-web.db.core` in some other namespace.
